...
def cumpleaños(token):
    ...
    for cumple in cumples:
        if (
            cumple.birthday.day == date.today().day
            and cumple.birthday.month == date.today().month
        ):
...
def cumpleaños_semana(token):
    ...
    for cumple in cumples:
        if (cumple.birthday.replace(year=date.today().year) - date.today()).days >= 0:
            ...
            days = (
                cumple.birthday.replace(year=date.today().year + 1) - date.today()
            ).days
            birthday = cumple.birthday.replace(year=date.today().year)

        if days > 0 and days <= 7:
            birthday = this_birthday(
                User.objects.get(customuser=cumple).first_name,
                User.objects.get(customuser=cumple).last_name,
                birthday,
            )
